...
public class Global extends ImporterTopLevel
{
    ...
    static Properties rhinodeProps = new Properties();
    static {
        try {
            rhinodeProps.load(Global.class.getResourceAsStream("/rhinode.properties"));
        } catch (IOException x) {
     ...
     * This is pretty hacked up.
     * TODO: use rhino's built-in require functionality
     */
    private static Map<String,Scriptable> modules = new HashMap<String,Scriptable>();
    public static Object require(Context cx, Scriptable thisObj,
                            Object[] args, Function funObj) throws IOException
    {
        Scriptable exports;
        String file = rhinodeProps.getProperty("modules")+"/"+
            Context.toString(args[0])+".js";
        if (modules.containsKey(file)) {
             exports = modules.get(file);
        } else {
            // make a new "global" object
            ...
            // execute the module
            Main.processFile(cx, scope, file);
